...
<p align="center">
  <img width="300" src="assets/image/image_YuWw3yyOp-.png">
</p>
...
4. **训练内循环（按Iteration进行）**：
   ...
   - **E. 参数更新：** 执行 **一次迭代**，更新模型参数：$\boldsymbol{\theta}_{k+1} \leftarrow \boldsymbol{\theta}_k - \mu_k \boldsymbol{g}_k$
...
7. **重复：** 算法返回步骤2（或步骤3，如果 $e < E\_{\text{max}}$ ），开始新一轮的Epoch（再次重排、再次迭代...），直到达到收敛标准（例如，完成 $E\_{\text{max}}$ 轮，或验证损失连续多轮不再下降）。
